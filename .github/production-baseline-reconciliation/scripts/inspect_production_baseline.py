...
def normalize_run_component(value: str) -> str:
...
def create_unique_run_dir(
    ...
    base = (
        f"{timestamp}-{normalize_run_component(production_tag)}-"
        f"{default_head[:12]}"
    )
...
def write_markdown(data: dict[str, object], output: Path) -> None:
    ...
    lines = [
        ...
        f"- Run directory: `{escape_markdown(data['run_directory'])}`",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            "Root directory for isolated analysis runs. A unique child directory is "
            "created for every execution."
        ),
    )
...
def main() -> int:
    ...
    try:
        ...
        markdown_path = run_dir / "production-baseline-evidence.md"
