...
1. Read its metadata from `production-baseline-evidence.md`.
...
7. Write a one-line reason based on the actual diff in
   `<RUN_DIR>/production-baseline-report.md`.

The heuristic hint emitted by the inspection script is never the final answer.
Do not create a JSON reviewed manifest.
